...
pub fn lineCountForWindow(win: vaxis.Window, message: []vaxis.Segment) usize {
    if (win.width == 0) return 0;
    var row: usize = 0;
    var col: usize = 0;
    var wrapped: bool = false;
    for (message) |segment| {
        var word_iter = ziglyph.WordIterator.init(segment.text) catch {
            // if it's not valid unicode, we'll just divide by length and maybe
            // we'll be wrong...oh well
            return std.math.divFloor(usize, segment.text.len, win.width) catch unreachable;
        };
